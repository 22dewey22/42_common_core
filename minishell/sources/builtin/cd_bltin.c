...
/*   Updated: 2024/05/28 09:15:27 by cdutel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	cd_home(t_mini *mini)
{
	...
	if (ft_strdup2(get_env_variable(mini->env, "PWD"), &oldpwd) == EXIT_FAILURE)
		return (ft_printf_fd(2, "%s", ERR_MSG_CD_FATAL_ERR),
...
static int	cd_choose_case(t_mini *mini, char **dir)
{
	...
	res = NULL;
	if (get_curr_dir_name(&res, 0) != EXIT_SUCCESS)
	...
	if (ft_strcmp("-", dir[1]) == 0 || ft_strcmp("~-", dir[1]) == 0)
	{
		...
		if (tmp == NULL || tmp[0] == '\0')
			return (mini->exit_status = 1, free(res),
				ft_printf_fd(2, ERR_MSG_CD_OLD), EXIT_SUCCESS);
...
int	cd_bltin(char **dir, t_mini *mini, int is_child)
{
	...
	if (dir[1] == NULL || (dir[1][0] == '~' && dir[1][1] == '\0'))
	{
		...
		ft_printf_fd(2, "%s", ERR_MSG_CD_ARG);
	else
	{
