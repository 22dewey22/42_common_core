...
/*   Updated: 2024/05/27 23:39:52 by cdutel           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	**add_line(char **env, char *to_add, t_mini *mini)
{
	...
	if (!new_env)
		return (export_malloc_error(mini), env);
	i = 0;
	while (env[i])
	{
		new_env[i] = ft_strdup(env[i]);
		if (!new_env[i])
			return (destroy_tab(&new_env), export_malloc_error(mini), env);
		i++;
	}
...
static void	check_and_test(t_mini *mini, char *to_add, int *error, int *trigger)
{
	*trigger = 0;
	if (*error == 1)
		...
		*error = check_to_add(to_add, trigger);
	if (*trigger != -1)
