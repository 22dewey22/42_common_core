...
/*   Updated: 2024/05/28 12:54:56 by dwayenbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	line_is_walid(t_split **first, int *open_par, t_mini *mini)
{
	...
	if (!current && *open_par == 0)
		...
		return (syntax_err_msg(SYNTAX_ERR_PAR_OPEN, "(", NULL));
	else if (current->tok_type == PIPE)
		return (line_is_walid_pipe(first, open_par, mini));
	else if (current->sub_tok_type == REDIR)
		return (line_is_walid_redir(first, open_par, mini));
	else if (current->sub_tok_type == CMD_END)
		return (line_is_walid_cmd_end(first, open_par, mini));
	else if (current->tok_type == PAR_OPEN)
		return (line_is_walid_par_open(first, open_par, mini));
	else if (current->tok_type == PAR_CLOSE)
		return (line_is_walid_par_close(first, open_par, mini));
	else
		return (line_is_walid_cmd(first, open_par, mini));
}
...
int	check_first_token(t_split *first, int *exit_status)
{
	t_tok_type	type;
	...
	else
	{
		*exit_status = SYNTAX_ERR;
		return (syntax_err_msg(SYNTAX_ERR, NULL, first->str));
	}
}
