...
/*   Updated: 2023/12/02 22:32:42 by dwayenbo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	printf_print_int(t_conv to_print, va_list args, int *count)
{
	...
	*count += to_print.end_len;
	...
	{
		if (to_print.sign != '\0')
			ft_putchar_fd(to_print.sign, to_print.fd);
...
void	printf_print_uint(t_conv to_print, va_list args, int *count, char *base)
{
	...
	*count += to_print.end_len;
	...
	if (ft_ischarset(to_print.conv, "xX") && to_print.hash == '#' && var)
	{
		...
		ft_putchar_fd(to_print.conv, to_print.fd);
	}
...
void	printf_print_char(t_conv to_print, va_list args, int *count)
{
	...
	*count += 1;
	i = 0;
	if (to_print.flag == '-')
		ft_putchar_fd((char)var, to_print.fd);
